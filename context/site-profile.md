...
- Verification ladder, all optional/pay-per-use. Without any ID verification, a user can still: create a profile, use Profile Import, control field-level privacy, share their profile (both Public View and Private View), and export their profile to PDF — none of that requires verification. What does require Level 1 ID Verification (photo ID + selfie + face scan) is any interaction with other users: searching, using Discover, requesting someone else's Private View, and chat all gate on Level 1 first. Level 2 Background Check (live video call reviewing personal/education/work history, either side can request once real conversation has started) and Level 3 In-Person Verification (local verifier confirms the profile in person) are not gates on interaction, just added credibility, on top of Level 1. Each gives a private shareable report. Credit history and criminal history checks are "coming soon," not live.
